...
include("Header_Clerk.php");
	...
	$Query = "Select * from 2017_2nd_computer_student_info where enr_no=".$_GET['enr_no'];
	...
	if(isset($_POST['Update']))
	{
		...
		$Query = "UPDATE `2017_2nd_computer_student_info` SET `enr_no`='$enr_no' ,`library_fees`='$library_fees', `mid_sem_fees`='$mid_sem_fees',`external_exam_fees`='$external_exam_fees' where `2017_2nd_computer_student_info`.`enr_no` = $enr_no;";
		
		mysql_query($Query) or die("<h2>$Query</h2> Query Error :".mysql_error());
		
		header("Location:SDisplay_2017_2nd_computer_info.php");
	}
?>
<div class="container">
