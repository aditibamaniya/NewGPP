...
include("Header_Admin.php");
	...
	$Query = "Select * from 2017_1st_computer_student_academic_info where enr_no=".$_GET['enr_no'];
	...
	if(isset($_POST['Update']))
	{
		...
		$Query = "UPDATE `2017_1st_computer_student_academic_info` SET `enr_no`='$enr_no',`name`='$name',`stream`='$stream',`current_sem`='$current_sem',`current_year`='$current_year',`ssc`='$ssc ',`hsc`='$hsc' where `2017_1st_computer_student_academic_info`.`enr_no` = $enr_no;";
		
		mysql_query($Query) or die("<h2>$Query</h2> Query Error :".mysql_error());
		
		header("Location:Display_2017_1st_computer_student_academic_info.php");
	}
?>
<div class="container">
