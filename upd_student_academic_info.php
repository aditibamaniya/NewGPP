...
include("Header.php");
	...
	$Query = "Select * from student_academic_info_master where enr_no=".$_GET['enr_no'];
	...
	if(isset($_POST['Update']))
	{
		...
		$Query = "UPDATE `student_academic_info_master` SET `enr_no`='$enr_no',`name`='$name',`stream`='$stream',`current_sem`='$current_sem',`current_year`='$current_year',`ssc`='$ssc ',`hsc`='$hsc' where `student_academic_info_master`.`enr_no` = $enr_no;";
		
		mysql_query($Query) or die("<h2>$Query</h2> Query Error :".mysql_error());
		
		header("Location:Display_student_academic_info.php");
	}
?>
<div class="container">
