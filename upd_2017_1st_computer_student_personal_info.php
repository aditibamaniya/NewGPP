...
include("Header_Admin.php");
	...
	$Query = "Select * from 2017_1st_computer_student_personal_info where enr_no=".$_GET['enr_no'];
	...
	if(isset($_POST['Update']))
	{
		...
		$Query = "UPDATE `2017_1st_computer_student_personal_info` SET `enr_no`='$enr_no',`name`='$name',`address`='$address',`birth_date`='$birth_date',`email`='$email',`category`='$category ',`mobile(self)`='$tel_self',`mobile(home)`='$tel_home',`district`='$district',`gender`='$gender' where `2017_1st_computer_student_personal_info`.`enr_no` = $enr_no;";
		
		mysql_query($Query) or die("<h2>$Query</h2> Query Error :".mysql_error());;
		
		header("Location:Display_2017_1st_computer_student_personal_info.php");
	}
?>
<div class="container">
